...
def showpoints_frame(xyz,c_gt=None, c_pred = None ,waittime=0,showrot=False,magnifyBlue=0,freezerot=False,background=(0,0,0),normalizecolor=True,ballradius=10):
    ...
    def render():
        ...
        # render airplane 2 unsuper 
        # xangle=-np.pi/8
        # yangle=-np.pi/4

        # render car 2 unsuper 
        # xangle=-np.pi/2
        # yangle=-np.pi/4

        # render chair 2 unsuper
        xangle = -np.pi/4
        yangle = -np.pi/4
...
if __name__=='__main__':
    np.random.seed(100)
    # showpoints(np.random.randn(2500,3),ballradius=8, normalizecolor=False, background=[255, 255, 255])
    frame = showpoints_frame(np.random.randn(2500,3),ballradius=8, normalizecolor=False, background=[255, 255, 255])
    print()
