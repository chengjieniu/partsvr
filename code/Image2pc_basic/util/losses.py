...
def regularization_loss(scopes, postfix=""):
    reg_loss = 0
    weight_decay = 0.001
    if weight_decay > 0:
        def is_weights(x):
            return 'weight' in x.name

        for scope in scopes:
            scope_vars = [x for x in tf.trainable_variables() if x.name.startswith(scope)]
            scope_vars_w = list(filter(is_weights, scope_vars))
            if scope_vars_w:
                reg_loss += tf.add_n([torch.nn.MSELoss(var) for var in scope_vars_w])

    #("losses/reg_loss" + postfix, reg_loss)
