...
def separable_kernels(kernel):
    size = kernel.shape[0]
    k1 = torch.reshape(kernel, [1, 1, size, 1, 1])
    k2 = torch.reshape(kernel, [1, size, 1, 1, 1])
    k3 = torch.reshape(kernel, [size, 1, 1, 1, 1])
    return [k1, k2, k3]
...
def smoothing_kernel(cfg, sigma):
    ...
    if cfg.vox_size_z != -1:
        ...
        k1 = torch.reshape(kernel_1d, [1, 1, fsz, 1, 1])
        k2 = torch.reshape(kernel_1d, [1, fsz, 1, 1, 1])
        k3 = torch.reshape(kernel_1d_z, [fsz_z, 1, 1, 1, 1])
        kernel = [k1, k2, k3]
    else:
