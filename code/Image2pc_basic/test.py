...
model_folder = os.path.join("testmodel//car_basic_pose")
...
for batch_idx, batch in enumerate(train_iter):
    ...
    # for i in range(0, 150):
        ...
        # plt.savefig(snapshot_folder+'/result_image_airplane_super_val/'+str(batch_idx)+'_'+str(i+1) +'_image.png')
        # plt.close()
        # plt.imshow(inputs['images'][3].squeeze(0).cpu().detach())
        # plt.savefig(snapshot_folder+'/result_image_airplane_super_val/'+str(batch_idx)+'_'+str(i+2) +'_image.png')
        # plt.close()
        print(batch_idx)
        ...
        plt.imshow(frame)
        plt.axis('off')
        plt.savefig(snapshot_folder+'/result_image_car_super_val/'+str(batch_idx)+'_dpc_'+str(i) +'_point1.png')
        plt.close()
        # frame = show_balls1.show_3d_point_clouds(outputs['points_1'][i].cpu().detach().numpy(), False)
        # plt.imshow(frame)
        # plt.axis('off')
        # plt.savefig(snapshot_folder+'/result_image_chair_super_test/'+str(batch_idx)+'_dpc_'+str(i) +'_point1.png')
        # plt.close()
...
print('done')
# scio.savemat(out_dir + synth_set + 'DPC_pred_test_code_test', {'name': points_pred['name'], 'points': points_pred['points']})  
print('ok')