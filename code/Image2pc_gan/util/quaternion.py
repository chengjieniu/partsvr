...
def quaternion_multiply(a, b):
    ...
    w1, x1, y1, z1 = torch.chunk(a, 4, dim=-1)
    w2, x2, y2, z2 = torch.chunk(b, 4, dim=-1)
    w = w1.squeeze(-1) * w2.squeeze(-1) - x1.squeeze(-1) * x2.squeeze(-1)- y1.squeeze(-1) * y2.squeeze(-1) - z1.squeeze(-1) * z2.squeeze(-1)
    x = w1.squeeze(-1) * x2.squeeze(-1) + x1.squeeze(-1) * w2.squeeze(-1) + y1.squeeze(-1) * z2.squeeze(-1) - z1.squeeze(-1) * y2.squeeze(-1)
    y = w1.squeeze(-1) * y2.squeeze(-1) + y1.squeeze(-1) * w2.squeeze(-1) + z1.squeeze(-1) * x2.squeeze(-1) - x1.squeeze(-1) * z2.squeeze(-1)
    z = w1.squeeze(-1) * z2.squeeze(-1) + z1.squeeze(-1) * w2.squeeze(-1) + x1.squeeze(-1) * y2.squeeze(-1) - y1.squeeze(-1) * x2.squeeze(-1)
    return torch.stack((w, x, y, z), dim=-1)


def quaternion_conjugate(q):
    """Compute the conjugate of q, i.e. [q.w, -q.x, -q.y, -q.z]."""
    return torch.mul(q, torch.cuda.FloatTensor([1.0, -1.0, -1.0, -1.0]))
...
def quaternion_rotate(pc, q, inverse=False):
    ...
    Returns:
        q * pc * q'
    """
    q_norm = torch.unsqueeze(torch.norm(q, p=2, dim =-1), dim =-1)
    q = q / q_norm
...
def from_rotation_matrix(mtr):
    ...
    def m(j, i):
        ...
        v = torch.index_select(mtr, begin=begin, size=size)
