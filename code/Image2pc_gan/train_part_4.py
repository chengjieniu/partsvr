...
def train():
    ...
    encoder_img_opt = torch.optim.Adagrad(encoder_img.parameters(), lr = 1e-3)
    decoder_pc_part_opt = torch.optim.Adagrad(decoder_pc_part.parameters(), lr = 1e-3)
    if config.predict_pose:
    ...
    for epoch in range(config.epochs):
        for batch_idx, batch in enumerate(train_iter):
            ...
            loss = total_loss  * 0.05 + gen_loss
            # loss = gen_loss
            plot_data.setdefault('iter', []).append(n_iter)
            plot_data.setdefault("purity_loss", []).append(loss_purity.item())
            plot_data.setdefault("total_loss", []).append(total_loss.item())
            plot_data.setdefault('loss', []).append(loss.item())
            plot_data.setdefault('gen_loss',[]).append(gen_loss.item())
            plot_data.setdefault('dis_loss', []).append(dis_loss.item())
        ...
        if epoch%100 == 0:
            scio.savemat(snapshot_folder  +'//'+'plot_purity'+str(epoch), {'iter':plot_data["iter"], 'purity_loss': plot_data['purity_loss'], \
                        'total_loss': plot_data['total_loss'], 'loss': plot_data['loss'], 'gen_loss': plot_data['gen_loss'],'dis_loss': plot_data['dis_loss'] })  
