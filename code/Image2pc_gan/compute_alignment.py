...
from util.quaternion_tf import as_rotation_matrix, from_rotation_matrix, quaternion_rotate
...
def alignment_to_ground_truth(pc_pred, quat_pred, gt_pred, quat_gt):
    ...
    """
    from util.quaternion_tf import quaternion_normalise, quaternion_conjugate, \
        quaternion_multiply
...
def compute_alignment_candidates(cfg, pred, gt_val, all_rotations_file):
    ...
    # num_show = 5
    rmse = np.ones((num_models, 4), np.float32)
    rotations = np.zeros((num_models, 4, 4), np.float32)
    ...
    for model_idx in range(num_models):
        ...
        for j in range (len(gt_val['name'][0])):
            if ''.join(pred['name'][model_idx])[0:32] == ''.join(gt_val['name'][0][j])[0:32]:
                ...
                for view_idx in range(num_views):
                    ...
                    # rotated_with_quat = quaternion_rotate(np.expand_dims(pred_pc_ref, 0), quat_pred.reshape(1,4))
                    # draw_registration_result(np.squeeze(rotated_with_quat), gt)
...
def compute_alignment():
    ...
    all_rotations_file = f"{exp_dir}/{synth_set}reference_rotations_partpose.mat"
    compute_alignment_candidates(cfg, pred, gt_val, all_rotations_file)
    ...
    scipy.io.savemat(f"{exp_dir}/{synth_set}final_reference_rotation.mat",
