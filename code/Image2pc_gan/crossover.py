...
print("Loading model ... ... ")
save_folder = os.path.join('application/crossover')
...
#leg
crossover1[0] = inter2[0]
crossover1[0, 512:768] = inter1[0, 512:768]

crossover2[0] = inter1[0]
crossover2[0, 512:768] = inter2[0, 512:768]
...
outputs1 = decoder_pc_part(inter1[0].unsqueeze(0).cuda(), config)
outputs1['points_1'] =  torch.cat((outputs1['pc_part_1'],outputs1['pc_part_2'],outputs1['pc_part_3'],outputs1['pc_part_4']),1)
outputs2 = decoder_pc_part(inter2[0].unsqueeze(0).cuda(), config)
outputs2['points_1'] =  torch.cat((outputs2['pc_part_1'],outputs2['pc_part_2'],outputs2['pc_part_3'],outputs2['pc_part_4']),1)

frame = show_balls.show_3d_point_clouds(outputs1['points_1'][0].cpu().detach().numpy(), False)
plt.imshow(frame)
plt.axis('off')
plt.savefig(save_folder+'/'+str(batch_idx)+'__output1' +'_point.png')
plt.close()

frame = show_balls1.show_3d_point_clouds(outputs1['points_1'][0].cpu().detach().numpy(), False)
plt.imshow(frame)
plt.axis('off')
plt.savefig(save_folder+'/'+str(batch_idx)+'__output1' +'_point1.png')
plt.close()

frame = show_balls.show_3d_point_clouds(outputs2['points_1'][0].cpu().detach().numpy(), False)
plt.imshow(frame)
plt.axis('off')
plt.savefig(save_folder+'/'+str(batch_idx)+'__output2' +'_point.png')
plt.close()

frame = show_balls1.show_3d_point_clouds(outputs2['points_1'][0].cpu().detach().numpy(), False)
plt.imshow(frame)
plt.axis('off')
plt.savefig(save_folder+'/'+str(batch_idx)+'__output2' +'_point1.png')
plt.close()
