...
def train():
   ...
   #     config.gpu = 0
    ...
    discri_part_3_opt = torch.optim.Adam(discri_part_3.parameters(), lr = 5e-8, weight_decay = 1e-8)
    discri_part_4_opt = torch.optim.Adam(discri_part_4.parameters(), lr = 5e-7, weight_decay = 1e-8)
    # discri_full_opt = torch.optim.Adam(discri_full.parameters(), lr = 1e-8, weight_decay = 1e-8)
    ...
    for epoch in range(config.epochs):
        for batch_idx, batch in enumerate(train_iter):
            ...
            # loss = total_loss +g1_loss +g2_loss +g3_loss +g4_loss +gfull_loss   
