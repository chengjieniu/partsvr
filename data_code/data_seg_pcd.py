...
synth_set = '02691156'
rootdir = '/media/ncj/Program/0retry_cv/shapenet_chair_pointcloud/02691156/pcd_seg_part/'
...
for i in range(0,len(list)):
    ...
    if list[i][-5] == '1':
        ...
        if len(points_part1['points'][-1])<2000:
            print('error')
        del points
    if list[i][-5] == '2':
        points = []
        ...
        if len(points_part2['points'][-1])<2000:
            print('error')
        del points
    if list[i][-5] == '3':
        points = []
        ...
        if len(points_part3['points'][-1])<2000:
            print('error')
        del points
    if list[i][-5] == '4':
        points = []
        ...
        if len(points_part4['points'][-1])<2000:
            print('error')
        del points   
f.close()
#chair
...
#airplane
#len(points_part1['name']) = 2689
#len(points_part2['name']) = 2683
#len(points_part3['name']) = 2665
#len(points_part4['name']) = 2153
...
for k in range(0,500):
    points_part4.setdefault('points', []).append(points)
    points_part4.setdefault('name',[]).append('0')


# fh5 = h5py.File(out_dir + synth_set + 'partnamegan.h5', 'w')
...
for i in range(0, len(points_part1['points'])):
    if len(points_part1['points'][i])<2000:
        print('error')
    fh5['points_part1'][i] = np.array(points_part1['points'][i], dtype = float)
for i in range(0, len(points_part2['points'])):
    if len(points_part2['points'][i])<2000:
        print('error')
    fh5['points_part2'][i] = np.array(points_part2['points'][i], dtype = float)
for i in range(0, len(points_part3['points'])):
    if len(points_part3['points'][i])<2000:
        print('error')
    fh5['points_part3'][i] = np.array(points_part3['points'][i], dtype = float)
for i in range(0, len(points_part4['points'])):
    if len(points_part4['points'][i])<2000:
        print('error')
    fh5['points_part4'][i] = np.array(points_part4['points'][i], dtype = float)
# fh5.close()
print('ok')

# fh5 = h5py.File(out_dir + synth_set + 'partnamegan.h5', 'w') #以'w'模式创建一个名为'test.h5'的HDF5对象ile(out_dir + synth_set + 'partgan.h5', 'w') #以'w'模式创建一个名为'test.h5'的HDF5对象
#scio.savemat(out_dir + synth_set + split_name, {"image":feature["image"], "mask":feature["mask"], "name":feature["name"]})
fh5.create_dataset('points_part1_name', (len(points_part1['name']), ), dtype = h5py.special_dtype(vlen=str))
fh5.create_dataset('points_part2_name', (len(points_part2['name']), ), dtype = h5py.special_dtype(vlen=str))
fh5.create_dataset('points_part3_name', (len(points_part3['name']), ), dtype = h5py.special_dtype(vlen=str))
fh5.create_dataset('points_part4_name', (len(points_part4['name']), ), dtype = h5py.special_dtype(vlen=str))
# fh5.create_dataset('mask', (num_views*len(models), im_size, im_size, 1), dtype = 'f4')
# fh5.create_dataset('name', (num_views*len(models), ), dtype = h5py.special_dtype(vlen=str))
for i in range(0, len(points_part1['points'])):
    fh5['points_part1_name'][i] = np.array(points_part1['name'][i], dtype = h5py.special_dtype(vlen=str))
for i in range(0, len(points_part2['points'])):
    fh5['points_part2_name'][i] = np.array(points_part2['name'][i], dtype = h5py.special_dtype(vlen=str))
for i in range(0, len(points_part3['points'])):
    fh5['points_part3_name'][i] = np.array(points_part3['name'][i], dtype = h5py.special_dtype(vlen=str))
for i in range(0, len(points_part4['points'])):
    fh5['points_part4_name'][i] = np.array(points_part4['name'][i], dtype = h5py.special_dtype(vlen=str))
fh5.close()
print('ok')
